...
public class JdbcAuthersRowSetTest {
   ...
   public static void main(String args[]) {
      ...
      try (JdbcRowSet rowSet = RowSetProvider.newFactory().createJdbcRowSet())            
         ...
         rowSet.setCommand("SELECT authorID, firstName, lastName FROM authors WHERE authorid > 3 ORDER BY firstName"); // set query
         rowSet.execute(); // execute query                      

         // process query results
         ResultSetMetaData metaData = rowSet.getMetaData();
         int numberOfColumns = metaData.getColumnCount();
         System.out.printf("Authors Table Row Set from Books Database:%n%n");
